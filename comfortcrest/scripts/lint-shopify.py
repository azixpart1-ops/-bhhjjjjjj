...
def check_render_filters(path, src):
    ...
    Shopify accepts the file, then drops it: the section or block simply stops
    existing, and the only visible symptom is a template that silently keeps
    its previous version. Three files were lost to this before the rule was
    written down. Hoist the value into an {% assign %} and pass the variable.
    """
    for m in re.finditer(r"{%-?\s*render\s+'[^']+'\s*,([^%]*?)-?%}", src):
        args = m.group(1)
        if '|' in args:
            err(path, "render argument uses a filter (%s) — assign it first"
                % args.strip()[:60])
